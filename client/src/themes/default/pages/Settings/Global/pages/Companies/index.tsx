import './index.scss';
import { defineComponent, type CreateElement } from 'vue';
...
import ModalBillingCompanyEdition from './modals';
...
type Data = {
  ...
  isFetching: boolean;
  isFetched: boolean;
  billingCompanies: BillingCompany[]; // To be defined
};
  ...
  methods: {
    ...
    async handleClickNewCompany() {
      const newCompany: BillingCompany | undefined = (
        await showModal(this.$modal, ModalBillingCompanyEdition)
      );
      if (!newCompany) {
        return;
      }
    ...
    async handleEditCompanyClick(e: MouseEvent, id: BillingCompany['id']) {
      ...
      const updatedCompany: BillingCompany | undefined = (
        await showModal(this.$modal, ModalBillingCompanyEdition, { company })
      );
      if (!updatedCompany) {
        return;
      }
    ...
    __(key: string, params?: Record<string, number | string>, count?: number): string {
      key = !key.startsWith('global.') ? `page.settings.companies.${key}` : key.replace(/^global\./, '');
