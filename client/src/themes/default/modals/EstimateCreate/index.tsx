...
import type { EstimateDetails } from '@/stores/api/estimates';
...
type Props = {
     ...
     *
     * @param estimate - Le devis créé si la création a été menée à son terme.
     */
    onClose?(estimate?: EstimateDetails): void,
};
...
/** Modale de création d'un devis "from scratch". */
const EstimateCreateModal = defineComponent({
    name: 'EstimateCreateModal',
    modal: {
        width: 940,
        dismissible: false,
    },
    ...
    methods: {
        ...
        async save() {
            ...
            try {
                const estimate = await apiEstimates.create({
    ...
    render() {
        ...
        return (
            <div class="EstimateCreateModal">
                <header class="EstimateCreateModal__header">
                    <h2 class="EstimateCreateModal__header__title">{__('title')}</h2>
                    <Button
                        type="close"
                        class="EstimateCreateModal__header__close-button"
                        onClick={handleClose}
                    />
                </header>
                <div class="EstimateCreateModal__stepper">
                    <Stepper
                        steps={steps}
                ...
                <div class="EstimateCreateModal__body">
                    {renderStep()}
                </div>
            </div>
        );
...
export default EstimateCreateModal;
