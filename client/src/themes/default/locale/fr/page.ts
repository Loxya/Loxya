export default {
    'login': require('@/themes/default/pages/Login/translations/fr.yml'),
    'schedule': require('@/themes/default/pages/Schedule/translations/fr.yml'),
    'event-edit': require('@/themes/default/pages/EventEdit/translations/fr.yml'),
    'event-departure': require('@/themes/default/pages/EventDeparture/translations/fr.yml'),
    'event-return': require('@/themes/default/pages/EventReturn/translations/fr.yml'),
    'users': require('@/themes/default/pages/Users/translations/fr.yml'),
    'user': require('@/themes/default/pages/UserEdit/translations/fr.yml'),
    'beneficiaries': require('@/themes/default/pages/Beneficiaries/translations/fr.yml'),
    'beneficiary-edit': require('@/themes/default/pages/BeneficiaryEdit/translations/fr.yml'),
    'beneficiary-view': require('@/themes/default/pages/BeneficiaryView/translations/fr.yml'),
    'invoices': require('@/themes/default/pages/Invoices/translations/fr.yml'),
    'estimates': require('@/themes/default/pages/Estimates/translations/fr.yml'),
    'company-edit': require('@/themes/default/pages/CompanyEdit/translations/fr.yml'),
    'materials': require('@/themes/default/pages/Materials/translations/fr.yml'),
    'material-edit': require('@/themes/default/pages/MaterialEdit/translations/fr.yml'),
    'material-view': require('@/themes/default/pages/MaterialView/translations/fr.yml'),
    'properties': require('@/themes/default/pages/Properties/translations/fr.yml'),
    'property-edit': require('@/themes/default/pages/PropertyEdit/translations/fr.yml'),
    'technicians': require('@/themes/default/pages/Technicians/translations/fr.yml'),
    'technician-edit': require('@/themes/default/pages/TechnicianEdit/translations/fr.yml'),
    'technician-view': require('@/themes/default/pages/TechnicianView/translations/fr.yml'),
    'roles': require('@/themes/default/pages/Roles/translations/fr.yml'),
    'parks': require('@/themes/default/pages/Parks/translations/fr.yml'),
    'park': require('@/themes/default/pages/ParkEdit/translations/fr.yml'),
    'settings': require('@/themes/default/pages/Settings/Global/translations/fr.yml'),
    'user-settings': require('@/themes/default/pages/Settings/User/translations/fr.yml'),
    'reset-password': require('@/themes/default/pages/ResetPassword/translations/fr.yml'),
};
