...
final class InvoiceMaterial extends BaseModel
{
    ...
    public function checkInvoiceId(mixed $value)
    {
        ...
        return !$this->exists || $this->isDirty('invoice_id')
            ? !$invoice->trashed()
            : true;
    }
    ...
    public function checkDegressiveRate(mixed $value)
    {
        ...
        // - La facture parente n'est pas récupérable, on est obligé
        ...
        // - Sinon, seules les factures non V1 sont concernées.
        } elseif ($this->invoice->format === BillingFormat::V1->value) {
            return V::nullType();
        }
    ...
    {
        // - Si la facture a une exemption globale, pas de taxe par ligne.
        if ($this->invoice->global_tax_regime !== null) {
            return null;
        }
        return $value;
    ...
    public function getTaxesAttribute(mixed $value): array|null
    {
        ...
        return array_map(
            function ($tax) {
                $isRate = $this->invoice->format < BillingFormat::V3->value
                    ? $tax['is_rate']
                    : true;
